...
export const SUBSCRIPTION_PLANS = {
  free: {
    ...
    limits: {
      ...
      taxExports: 1,
      teamMembers: 1,
      features: ['receipt_scanner_limited', 'excel_importer_limited', 'tax_export_limited']
    }
  },
  professional: {
    id: 'professional',
    ...
    features: [
      ...
      'Unlimited VAT tax exports'
    ],
    limits: {
      purchaseEntries: -1, // unlimited
      receiptScans: -1, // unlimited
      excelImports: -1, // unlimited
      taxExports: -1, // unlimited
      teamMembers: 1,
      features: [
        'receipt_scanner',
        'excel_importer',
...
export const isFeatureAvailable = (userPlan, feature) => {
  switch (feature) {
    case 'excelImports':
      return getUserPlanLimits(userPlan).excelImports > 0 || getUserPlanLimits(userPlan).excelImports === -1;
    case 'receiptScanner':
      return getUserPlanLimits(userPlan).receiptScans > 0;
    case 'taxExports':
      return getUserPlanLimits(userPlan).taxExports > 0 || getUserPlanLimits(userPlan).taxExports === -1;
    case 'purchaseEntries':
      return getUserPlanLimits(userPlan).purchaseEntries > 0 || getUserPlanLimits(userPlan).purchaseEntries === -1;
    default:
