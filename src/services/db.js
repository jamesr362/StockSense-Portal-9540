...
// Enhanced createUser function with better error handling and debugging
  ...
  try {
    ...
    if (supabaseAvailable()) {
      ...
      try {
        ...
        console.log('User created successfully in Supabase:',{...result,password: '[HIDDEN]'});
        return result;
      } catch (supabaseError) {
        console.log('Supabase creation failed, falling back to IndexedDB:',supabaseError.message);
        // Don't throw here, fall back to IndexedDB
      }
    } else {
    ...
    // Fallback to IndexedDB
    console.log('Creating user in IndexedDB...');
    ...
    const newUser={
      ...
      salt: userData.salt, // Store the salt if provided
    ...
    console.log('Adding user to IndexedDB:',{...newUser,password: '[HIDDEN]',salt: '[HIDDEN]'});
    ...
    console.log('User created successfully in IndexedDB:',userWithoutPassword);
...
// Enhanced getUserByEmail function with better debugging
  ...
  try {
    ...
    if (supabaseAvailable()) {
      ...
      try {
        ...
        if (user) {
          console.log('User found in Supabase:',{...user,password: '[HIDDEN]'});
          return user;
        }
        console.log('User not found in Supabase, checking IndexedDB...');
      } catch (supabaseError) {
        console.log('Supabase query failed, falling back to IndexedDB:',supabaseError.message);
      }
    } else {
      console.log('Supabase not available, checking IndexedDB...');
    }
    ...
    const user=await store.get(normalizedEmail);
    await tx.done;
    
    if (user) {
      console.log('User found in IndexedDB:',{...user,password: '[HIDDEN]'});
    } else {
